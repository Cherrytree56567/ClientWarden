...
bool AcrylicCompositor::CreateOverlayLayer(IDXGISwapChain1** ppSwapChain)
{
    ...
    if (dxgiFactory->CreateSwapChainForComposition(
            dxgiDevice.Get(), &desc, nullptr,
            overlaySwapChain.GetAddressOf()) != S_OK)
        return false;

    dcompDevice3->CreateVisual(&overlayVisual);
    overlayVisual->SetContent(overlaySwapChain.Get());

    containerVisual->AddVisual(overlayVisual.Get(), true, rootVisual.Get());
    Commit();

    overlaySwapChain.CopyTo(ppSwapChain);
    return true;
}

bool AcrylicCompositor::CreateCompositionDevice()
{
	if (D3D11CreateDevice(0, D3D_DRIVER_TYPE_HARDWARE, NULL,D3D11_CREATE_DEVICE_BGRA_SUPPORT, NULL, 0, D3D11_SDK_VERSION, d3d11Device.GetAddressOf(), nullptr, d3d11Context.GetAddressOf()) != S_OK)
	{
		return false;
	}
...
{
	dcompDevice3->CreateVisual(&containerVisual);
	...
	if (dcompTarget->SetRoot(containerVisual.Get()) != S_OK) {
        return false;
	}

    containerVisual->AddVisual(rootVisual.Get(), false, NULL);
